...
class GoogleDriveAsyncService:
    ...
    async def download(self, file_id: str) -> bytes:
        async with Aiogoogle(service_account_creds=self._creds) as aiogoogle:
            ...
            response = await aiogoogle.as_service_account(
                drive_v3.files.get(fileId=file_id, alt="media"),
                full_res=True
            )
    ...
    @timer
    async def create_file(self, metadata: dict, content: bytes, drive, aiogoogle) -> str:
    ...
    @timer
    async def update_file(self, file_id: str, metadata: dict, content: bytes, drive, aiogoogle) -> str:
        file = await aiogoogle.as_service_account(
            drive.files.update(
                fileId=file_id,
                json=metadata,
    ...
    async def create_folder_structure(self, name: str) -> str:
        ...
        async with Aiogoogle(service_account_creds=self._creds) as aiogoogle:
            ...
            for folder_name in path_parts:
                parent_id = await self._get_or_create_folder(folder_name, parent_id, aiogoogle, drive_v3)

        return parent_id

    async def _get_or_create_folder(self, folder_name: str, parent_id: str, aig, d) -> str:
        query = (
    ...
    async def convert_docx_to_pdf(self, file_id: str, filename: str, folder_id: str):
        ...
        async with Aiogoogle(service_account_creds=self._creds) as aiogoogle:
            ...
            try:
                ...
                # file = await self.get_file_by_name(pdf_name, folder_id, aiogoogle, drive_v3)
                # if file:
                #     await self.update_file(
                #         file_id=file['id'],
    ...
    async def get_file_by_name(self, filename: str, parent_id: str, aiogoogle, drive_v3) -> dict | None:
